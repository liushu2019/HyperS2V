...
def returnPathHyperS2V():
    return dir_f

def returnPathPickles():
    return config.folder_pickles

def isPickle(fname):
    return os.path.isfile(config.folder_pickles + fname+'.pickle')
...
def restoreVariableFromDisk(name):
    ...
    t0 = time()
    val = None
    with open(config.folder_pickles + name + '.pickle', 'rb') as handle:
...
def saveVariableOnDisk(f,name):
    ...
    t0 = time()
    with open(config.folder_pickles + name + '.pickle', 'wb') as handle:
