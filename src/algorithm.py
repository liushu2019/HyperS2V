...
def generate_random_walks_large_graphs(num_walks,walk_length,workers,vertices,suffix):
    ...
    save_random_walks(walks,suffix)

def generate_random_walks(num_walks,walk_length,workers,vertices,suffix):
...
def save_random_walks(walks,suffix):
    with open('pickles_{}/random_walks.txt'.format(suffix), 'w') as file:
        for walk in walks:
            line = ''
            for v in walk:
                line += str(v)+' '
